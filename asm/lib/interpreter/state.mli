...
module GlobalState :
  sig
    ...
    val initialize_registers_with :
      (string -> bool) -> 'a -> 'a Containers.StringMap.t
    val initialize_registers : int Containers.StringMap.t
    val initialize_xmm_registers : (int * int) Containers.StringMap.t
    val initial_state : t
    val update :
      ('a -> ('a, 'a) StateErrorMonad.t) -> ('a, unit) StateErrorMonad.t
    val show : t -> string
  end
...
val read_generic_reg :
  'a Ast.register ->
  ('b -> 'c Containers.StringMap.t) -> ('b, 'c) StateErrorMonad.t
val read_reg : 'a Ast.register -> (GlobalState.t, int) StateErrorMonad.t
val read_xmm_reg :
  'a Ast.register -> (GlobalState.t, int * int) StateErrorMonad.t
val write_reg :
  'a Ast.register -> int -> (GlobalState.t, unit) StateErrorMonad.t
val write_xmm_reg :
  'a Ast.register -> int * int -> (GlobalState.t, unit) StateErrorMonad.t
